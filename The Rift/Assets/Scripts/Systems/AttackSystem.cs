...
namespace Systems
{
    ...
    public void PerformEnemyAttack(IAttackProfile profile, EnemyModel enemyModel, GameObject sender, Teams team)
    {
        ...
        if (profile is RaycastAttackProfile raycastProfile)
        {
            Vector3 rayOrigin = sender.transform.position + sender.transform.rotation * raycastProfile.Offset;
            _coroutineRunner.StartRoutine(
                CastEnemyRaysContinuous(raycastProfile, enemyModel, sender, team)
            );
        }
        ...
        if (profile is CompositeAttackProfile compositeProfile)
        {
            foreach (var timing in compositeProfile.AttackTimings)
            {
                PerformEnemyAttack(timing.Attack.Value, enemyModel, sender, team);
                Waiter.WaitForSec(timing.Timing);
            }
        }
        ...
        if (profile is AttackSwitch attackSwitch)
        {
            PerformEnemyAttack(attackSwitch.GetNextAttack(), enemyModel, sender, team);
        }
        ...
        if (profile is CircularAttackProfile circularProfile)
        {
            ...
            foreach (var col in colliders)
            {
                ...
                if (target == null) continue;
                if (target.Team == team) continue;
                target.Health.TakeDamage(enemyModel.Damage, circularProfile.DamageType);
                
                if (!circularProfile.Piercing) break;
            }
        }   
    }
    private IEnumerator CastEnemyRaysContinuous(RaycastAttackProfile attackProfile, EnemyModel enemyModel, GameObject sender, Teams team) 
        {
            var range = enemyModel.AttackDistance * attackProfile.DistanceMultiplier;
            var damage = enemyModel.Damage * attackProfile.DamageMultiplier;
            var piercing = true;
            var swingSpeed = 5f; 
            var totalAngle = attackProfile.Angle;            
            var tilt = attackProfile.Tilt;                    
            ...
            for (float angle = -halfAngle; angle <= halfAngle; angle += 1f)
            {
                ...
                lr.endWidth = 0.1f;
                lr.startColor = Color.red;
                lr.endColor = Color.cornflowerBlue; 
                CoroutineRunner.Destroy(lineObj, 0.1f);
                //конец отрисовки
                #endregion
